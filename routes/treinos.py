...
from models.treino_exercicio_embedded import ExercicioTreinoEmbedded, ExercicioTreinoInput
...
async def add_exercicio_to_treino(treino_id: str, exercicio_input: ExercicioTreinoInput) -> Treino:
    ...
    exercicio = await engine.find_one(Exercicio, Exercicio.id == ObjectId(exercicio_input.exercicio_id))
    if not exercicio:
...
async def update_treino(treino_id: str, aluno_id: str, treino_in: TreinoInput) -> Treino:
    treino = await engine.find_one(Treino, Treino.id == ObjectId(treino_id))
    if not treino:
        raise HTTPException(status_code=404, detail="Treino not found")
    aluno = await engine.find_one(Aluno, Aluno.id == ObjectId(aluno_id))
...
async def remove_exercicio_from_treino(treino_id: str, exercicio_id: str) -> Treino:
    ...
    exercicios_atualizados = [ex for ex in treino.exercicios if ex.exercicio_id != exercicio_id]
